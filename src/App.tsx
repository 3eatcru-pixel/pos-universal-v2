...
import { MOCK_PRODUCTS, MOCK_TABLES, MOCK_INVENTORY, MOCK_STAFF, MOCK_SHIFTS, MOCK_PERMISSIONS, MOCK_PRINTERS, MOCK_SHOPS, MOCK_ENTERPRISE, MOCK_ORDERS, MOCK_BUSINESS_CONFIG, MOCK_SCHEDULES } from './mockData';
...
export default function App() {
  ...
  const [isLocalMode, setIsLocalMode] = useState<boolean>(() => {
    return localStorage.getItem('rm_local_mode') === 'true';
  });
  ...
  useEffect(() => {
    ...
    if (isLocalMode) {
      ...
      setRolePermissions(MOCK_PERMISSIONS);
      setPrinters(MOCK_PRINTERS);
      setOrders(MOCK_ORDERS);
      setBusinessConfigs(MOCK_BUSINESS_CONFIG);
      setStaffSchedules(MOCK_SCHEDULES);
      return () => globalUnsubs.forEach(u => u());
    }
  ...
  useEffect(() => {
    ...
    else localStorage.removeItem('rm_enterprise_id');
  }, [enterpriseId]);
  ...
  useEffect(() => {
    ...
    // Load Local Data if Offline/LocalMode
    const loadLocalData = async () => {
      if (isLocalMode) {
        const localOrders = await dbLocal.get('orders');
        const localTables = await dbLocal.get('tables');
        const localInventory = await dbLocal.get('inventory');
        if (localOrders) setOrders(localOrders);
        if (localTables) setTables(localTables);
        if (localInventory) setInventory(localInventory);
      }
    };
    loadLocalData();
  }, [isLocalMode]);

  // Sync state to IndexedDB when it changes in LocalMode
  useEffect(() => {
    if (isLocalMode) {
      dbLocal.set('orders', orders);
      dbLocal.set('tables', tables);
      dbLocal.set('inventory', inventory);
    }
  }, [orders, tables, inventory, isLocalMode]);
  ...
  const [tempPayments, setTempPayments] = useState<{method: 'cash'|'card'|'pix', amount: number, change?: number}[]>([]);
  
  // History & Pending Orders Filters
  const [historyFilter, setHistoryFilter] = useState<'all' | 'open' | 'closed'>('all');
  const [pendingOrderFilter, setPendingOrderFilter] = useState<'all' | 'to_kitchen' | 'waiting_payment'>('all');
  const [pendingOrderSort, setPendingOrderSort] = useState<'time' | 'value' | 'table'>('time');
  const [reportsTab, setReportsTab] = useState<'summary' | 'sales' | 'products' | 'staff'>('summary');
  const [reportsStartDate, setReportsStartDate] = useState(format(startOfWeek(new Date()), 'yyyy-MM-dd'));
  ...
  const handleOpenTable = async (table: Table) => {
    if (table.status === 'free' || table.status === 'reserved') {
      ...
      if (isLocalMode) {
        ...
        setTables(prev => prev.map(t => t.id === table.id ? { ...t, status: 'occupied', currentOrderId: orderId } : t));
...
Obrigado pela preferência!
  ...
  const handlePlaceOrder = async () => {
    ...
    const serviceFee = isTakeaway ? 0 : (subtotal * 0.10); // Standard 10% service fee for tables
    ...
    if (isLocalMode) {
      setOrders(prev => {
        ...
        return [...otherOrders, orderData];
      });
      if (!isTakeaway) {
        setTables(prev => prev.map(t => t.id === selectedTable.id ? { ...t, status: 'occupied', currentOrderId: orderId } : t));
      }
      meshNetwork.broadcast('order:update', orderData);
  ...
  const handleApplyDiscount = (amount: number) => {
    ...
    setOrders(prev => prev.map(o => {
      if (o.tableId === selectedTable.id && o.status !== 'delivered') {
        ...
        const newTotal = Math.max(0, subtotal + serviceFee - amount);
        return { ...o, discount: amount, total: newTotal };
      }
      return o;
    }));
  };
  ...
  const processFinalPayment = async (orderId: string, payments: {method: 'cash'|'card'|'pix', amount: number, change?: number}[]) => {
    const order = orders.find(o => o.id === orderId);
    if (!order) return;

    // Check if it's a takeaway that was pending - now send to kitchen
    const shouldSendToKitchen = order.orderType === 'takeaway' && order.status === 'pending';
    ...
    const updates = {
      items: updatedItems,
      status: shouldSendToKitchen ? 'preparing' : 'delivered' as OrderStatus,
      closedAt: Date.now(),
      paymentMethod: (payments.length > 1 ? 'split' : payments[0].method) as any,
      payments: payments
    };
    ...
    if (isLocalMode) {
      ...
      if (order.tableId && order.tableId !== 'takeaway') {
        setTables(prev => prev.map(t => t.id === order.tableId ? { ...t, status: 'free', currentOrderId: undefined } : t));
      }
    ...
    if (shouldSendToKitchen) {
       alert(`Pagamento Confirmado! Pedido #${order.takeawayNumber} enviado para a cozinha.`);
    }
  ...
  const renderPaymentModal = () => {
    ...
    const paidAmount = tempPayments.reduce((sum, p) => sum + p.amount, 0);
    const balanceRemaining = Math.max(0, paymentOrder.total - paidAmount);
    const isFullyPaid = paidAmount >= paymentOrder.total;
    const change = paidAmount > paymentOrder.total ? paidAmount - paymentOrder.total : 0;
        ...
        {isPaymentModalOpen && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
            ...
            >
              <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <div>
                  <h3 className="text-xl font-black text-slate-800 tracking-tight">Finalizar Pagamento</h3>
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">
                    {paymentOrder.orderType === 'takeaway' 
                      ? `Takeaway #${paymentOrder.takeawayNumber}` 
                      : `Mesa 0${tables.find(t => t.id === paymentOrder.tableId)?.number}`} 
                    • Pedido #{paymentOrder.id.substr(-6).toUpperCase()}
                  </p>
                </div>
                <button onClick={() => setIsPaymentModalOpen(false)} className="w-10 h-10 flex items-center justify-center rounded-full bg-white shadow-sm text-slate-400 hover:text-slate-600 border border-slate-100 transition-all">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="p-8 space-y-8 overflow-y-auto">
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-900 rounded-2xl p-6 text-white shadow-xl shadow-slate-900/20">
                     <p className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] mb-2">Total a Pagar</p>
                     <p className="text-3xl font-black">{formatCurrency(paymentOrder.total)}</p>
                  </div>
                  <div className={cn(
                    "rounded-2xl p-6 shadow-xl transition-all flex flex-col justify-center",
                    isFullyPaid ? "bg-emerald-500 text-white shadow-emerald-500/20" : "bg-slate-50 text-slate-400 border border-slate-100"
                  )}>
                     <p className="text-[10px] font-black uppercase opacity-60 tracking-[0.2em] mb-1">
                       {change > 0 ? 'Troco a Devolver' : 'Balanço Restante'}
                     </p>
                     <p className="text-3xl font-black">
                       {formatCurrency(change > 0 ? change : balanceRemaining)}
                     </p>
                  </div>
                </div>
                ...
                <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100 space-y-2">
                  <div className="flex justify-between text-[10px] font-black uppercase text-slate-400">
                    <span>Subtotal</span>
                    <span className="text-slate-600">{formatCurrency(paymentOrder.subtotal)}</span>
                  </div>
                  <div className="flex justify-between text-[10px] font-black uppercase text-slate-400">
                    <span>Taxa de Serviço (10%)</span>
                    <span className="text-slate-600">{formatCurrency(paymentOrder.serviceFee || 0)}</span>
                  </div>
                  {paymentOrder.discount > 0 && (
                    <div className="flex justify-between text-[10px] font-black uppercase text-rose-400">
                      <span>Desconto</span>
                      <span className="text-rose-500">-{formatCurrency(paymentOrder.discount)}</span>
                    </div>
                  )}
                  <div className="pt-2 border-t border-slate-200 flex justify-between text-xs font-black uppercase text-slate-800">
                  ...
                  <div className="space-y-4">
                    <div className="bg-emerald-50 rounded-2xl p-4 border border-emerald-100">
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="text-[10px] font-black uppercase text-emerald-600 tracking-[0.2em]">Divisão Rápida</h4>
                      ...
                      <div className="grid grid-cols-4 gap-2">
                        {[2, 3, 4, 5].map(n => (
                          <button 
                            key={n}
                            onClick={() => {
                              const val = paymentOrder.total / n;
                              if (confirm(`Dividir em ${n} pessoas? Valor por pessoa: ${formatCurrency(val)}. Adicionar agora como pagamento no cartão?`)) {
                                setTempPayments([...tempPayments, { method: 'card', amount: Number(val.toFixed(2)) }]);
                              }
                            }}
                            className="bg-white border border-emerald-100 py-2 rounded-lg text-xs font-bold text-emerald-600 hover:bg-emerald-500 hover:text-white transition-all shadow-sm"
                          >
                            {n}x
                          </button>
                        ))}
                    ...
                    <h4 className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] px-1">Método de Pagamento</h4>
                    <div className="grid grid-cols-3 gap-3">
                      {[
                        { id: 'card', label: 'Cartão', icon: <CreditCard className="w-5 h-5" />, color: 'blue' },
                        { id: 'pix', label: 'PIX', icon: <Smartphone className="w-5 h-5" />, color: 'emerald' },
                        { id: 'cash', label: 'Dinheiro', icon: <Banknote className="w-5 h-5" />, color: 'amber' },
                      ].map(method => (
                        <button 
                          key={method.id}
                          onClick={() => {
                            if (method.id === 'cash') {
                               const val = prompt("Valor recebido em dinheiro:");
                               if (val) {
                                  const amount = parseFloat(val);
                                  if (amount >= balanceRemaining) {
                                     setTempPayments([...tempPayments, { method: 'cash', amount, change: amount - balanceRemaining }]);
                                  } else {
                                     setTempPayments([...tempPayments, { method: 'cash', amount }]);
                                  }
                               }
                            } else {
                               const val = prompt(`Valor no ${method.label}:`, balanceRemaining.toString());
                               if (val) setTempPayments([...tempPayments, { method: method.id as any, amount: parseFloat(val) }]);
                            }
                          }}
                          className={cn(
                            "flex flex-col items-center justify-center gap-3 p-5 rounded-2xl border transition-all group lg:min-h-[100px]",
                            method.id === 'card' ? "bg-blue-50/50 border-blue-100 hover:bg-blue-500 hover:text-white hover:border-blue-500" :
                            method.id === 'pix' ? "bg-emerald-50/50 border-emerald-100 hover:bg-emerald-500 hover:text-white hover:border-emerald-500" :
                            "bg-amber-50/50 border-amber-100 hover:bg-amber-500 hover:text-white hover:border-amber-500"
                          )}
                        >
                          <div className={cn("p-2 rounded-lg transition-colors", 
                            method.id === 'card' ? "bg-blue-100 text-blue-600 group-hover:bg-white/20 group-hover:text-white" :
                            method.id === 'pix' ? "bg-emerald-100 text-emerald-600 group-hover:bg-white/20 group-hover:text-white" :
                            "bg-amber-100 text-amber-600 group-hover:bg-white/20 group-hover:text-white"
                          )}>
                            {method.icon}
                          </div>
                    ...
                    <button 
                      onClick={() => {
                        const val = prompt("Valor customizado a abater:");
                        if (val) setTempPayments([...tempPayments, { method: 'card', amount: parseFloat(val) }]);
                      }}
                      className="w-full py-4 border-2 border-dashed border-slate-200 rounded-2xl text-slate-400 font-bold text-xs uppercase tracking-widest hover:border-emerald-300 hover:text-emerald-500 transition-all flex items-center justify-center gap-2"
                    >
                      <ArrowLeftRight className="w-4 h-4" /> Pagamento Parcial
                    </button>
                  </div>
                )}
                ...
                {tempPayments.length > 0 && (
                  <div className="space-y-3">
                    <h4 className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] px-1">Pagamentos Efetuados</h4>
                         ...
                         <div key={idx} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100">
                            <div className="flex items-center gap-3">
                              <div className="w-8 h-8 rounded-lg bg-white shadow-sm flex items-center justify-center text-slate-400">
                                {p.method === 'card' ? <CreditCard className="w-4 h-4" /> : p.method === 'pix' ? <Smartphone className="w-4 h-4" /> : <Banknote className="w-4 h-4" />}
                              </div>
                              <span className="text-xs font-bold text-slate-700 uppercase tracking-tight">{p.method}</span>
                            </div>
                            <div className="text-right">
                               <p className="text-sm font-black text-slate-800">{formatCurrency(p.amount)}</p>
                               {p.change !== undefined && p.change > 0 && <p className="text-[9px] font-bold text-amber-600 uppercase">Troco: {formatCurrency(p.change)}</p>}
                            </div>
                         </div>
                       ))}
                    </div>
              ...
              <div className="p-8 bg-slate-50 border-t border-slate-100 mt-auto">
                <button 
                  ...
                  onClick={() => processFinalPayment(paymentOrder.id, tempPayments)}
                  className={cn(
                    "w-full py-5 rounded-2xl font-black text-sm uppercase tracking-[0.2em] transition-all shadow-xl",
                    isFullyPaid ? "bg-emerald-500 text-white shadow-emerald-500/30 hover:bg-emerald-400 active:scale-[0.98]" : "bg-slate-200 text-slate-400 cursor-not-allowed"
                  )}
                >
           ...
           <div className="sleek-card p-8 bg-white border border-slate-100 shadow-sm">
              ...
              <div className="space-y-4 max-w-md">
                 <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl">
                    <div>
                      <p className="text-xs font-bold text-slate-800 tracking-tight">Taxa de Serviço Automática</p>
                      <p className="text-[9px] text-slate-400 font-medium">Sugerir 10% no fechamento da conta</p>
                    </div>
                    <div className="w-10 h-10 bg-emerald-500 rounded-full flex items-center justify-center text-white font-black text-[10px]">ON</div>
                 </div>
